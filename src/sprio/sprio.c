...
uint32_t weight_age_time; /* weight for AgeTime factor */
...
int main (int argc, char **argv)
{
	...
	if (working_cluster_rec) {
		...
		weight_age_time = slurm_ctl_conf_ptr->priority_weight_age_time;
	...
	if (params.format == NULL) {
		if (params.normalized) {
			if (params.long_list) {
				params.format = "%.15i %9r %.8u %10y %10a %10b "
					"%10f %10j %10p %10q %20t";
			} else {
			...
			if (params.long_list) {
				params.format = "%.15i %9r %.8u %.10Y %.10A "
					"%.10B %.10F %.10J %.10P %.10Q %.11N "
					"%.20T";
			} else {
				params.format = xstrdup("%.15i %9r");
				if (params.sibling && !params.local)
